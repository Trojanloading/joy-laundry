<template>
  <!-- 删除会员充值记录 -->
  <div class="deleteRecord-container">
    <el-dialog title="删除会员充值记录" top="30vh" :visible.sync="dialogFormVisible">
      <div class="title">此操作将永久删除该会员充值记录, 是否继续?</div>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          ...
          round
          :loading="loading"
          @click.native.prevent="handleDeleteRecord"
        >{{ btnText }}</el-button>
      </div>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleDeleteRecord() {
      ...
      axios
        .post(
          "http://127.0.0.1:3000/memberConsumptionStatistics/deleteRecord",
        ...
        .then(res => {
          ...
          if (res.data == true) {
            ...
            console.log("删除会员卡充值记录成功");
          } else {
            this.loading = false;
            this.btnText = "确 定";
            console.log("删除会员卡充值记录失败");
            return false;
          }
        })
        .catch(error => {
